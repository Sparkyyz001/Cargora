...
interface PillNavProps {
  /** Логотип. Если не указан — не рендерится (логотип может быть снаружи). */
  logo?: string
  logoAlt?: string
  items: PillNavItem[]
  /** Дополнительные action-кнопки справа. Если не указаны — не рендерятся. */
  actions?: PillNavItem[]
  activeHref?: string
  className?: string
  ease?: string
...
const isRouterLink = (href?: string): boolean => Boolean(href && !isExternalLink(href))
  ...
  return (
    <div className="pill-nav-container">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        ...
        {/* Action-кнопки справа (Войти / Начать бесплатно) */}
        {actions.length > 0 && (
          <div className="pill-nav-actions desktop-only">
            {actions.map((action, i) =>
              isRouterLink(action.href) ? (
                <Link
                  key={action.href || `action-${i}`}
                  href={action.href}
                  className={`pill-action${i === actions.length - 1 ? " pill-action-primary" : ""}`}
